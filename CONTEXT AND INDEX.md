...
- **_build/**: primary source. `build.js` compiles the source into `app/index.html`. `icons.js` draws the icons. `serve.js` is a local test server. `deploy.cmd` builds, commits and pushes. `vendor/` holds React, `node_modules/` holds Babel, both copied from Macro so the build runs offline.
